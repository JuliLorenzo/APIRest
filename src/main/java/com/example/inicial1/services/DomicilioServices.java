...
public class DomicilioServices implements BaseService<Domicilio> {
    ...
    public Domicilio findById(Long id) throws Exception {
        try {
            Optional<Domicilio> retrievedDomicilio = domicilioRepository.findById(id);
            return retrievedDomicilio.get();
        } catch (Exception e){
            throw new Exception(e.getMessage());        }
    }
    ...
    public Domicilio update(Long id, Domicilio entity) throws Exception {
        try{
            Optional<Domicilio> retrievedDomicilio = domicilioRepository.findById(id);
            Domicilio newDomicilio = retrievedDomicilio.get();
            newDomicilio = domicilioRepository.save(entity);
            return newDomicilio;
        } catch (Exception e){
            throw new Exception(e.getMessage());
        }
